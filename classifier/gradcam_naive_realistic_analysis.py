#!/usr/bin/env python
"""
Grad-CAM interpretability analysis for the Naive-only and Realistic-only
ConvNeXt-base experiments, k=1 (train_size=1), split 1 (first random-eval split),
evaluated on real test fragments.

Produces two groups of heatmaps per model:
...
Usage:
    python gradcam_naive_realistic_analysis.py --model_path 1_groundtruth_swap_convnext_base_gt_rand_convnext_base_naive.pth --out_dir gradcam_naive_k1_split1
    python gradcam_naive_realistic_analysis.py --model_path 1_groundtruth_swap_convnext_base_gt_rand_convnext_base_realistic.pth --out_dir gradcam_realistic_k1_split1
"""
import os
import argparse
import numpy as np
...
class GradCAM:
    """Grad-CAM hooked on the last conv feature map before global pooling."""
...
def get_target_layer(model, model_name):
    if model_name == 'convnext_base':
        return model.convnext_base_ft.features[-1]
    elif model_name == 'convnextv2_base':
        return model.convnextv2_base_ft.stages[-1]
    else:
        raise ValueError(f"No known Grad-CAM target layer for model_name={model_name}")
...
def main():
    ...
    parser.add_argument('--model_path', required=True,
                         help='e.g. 1_groundtruth_swap_convnext_base_gt_rand_convnext_base_naive.pth')
    parser.add_argument('--test_csv',
                         default=os.path.join(
                             '1_groundtruth_swap_convnext_base_gt_rand',
                             '1_groundtruth_swap_convnext_base_gt_rand_1',
                             'test.csv'))
