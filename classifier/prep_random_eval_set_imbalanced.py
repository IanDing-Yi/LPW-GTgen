...
EXTRA_FILES = ("naive.csv", "realistic.csv")


def prep_random_eval_set_imbalanced(
    ...
    for file_name in EXTRA_FILES:
        shutil.copy2(os.path.join(input_folder, file_name), os.path.join(output_folder, file_name))

    print(
        f"Created imbalanced random eval set in {output_folder}: "
...
def main():
    if len(sys.argv) != 6:
        print(
            "Usage: python prep_random_eval_set_imbalanced.py "
            "<base_path> <gt_input_folder> <gt_output_folder> "
            "<train_size_per_class> <random_seed>"
        )
        sys.exit(1)

    prep_random_eval_set_imbalanced(
        sys.argv[1],
        sys.argv[2],
        sys.argv[3],
        int(sys.argv[4]),
